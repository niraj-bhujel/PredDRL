...
class GatedGCNLayer(nn.Module):
    """
        Param: []
    """
    def __init__(self, input_dim, output_dim, dropout, batch_norm, activation, residual=False):
        super().__init__()
...
# this module is modified by niraj
class CustomGatedGCNLayer(nn.Module):
    ...
    def message_func(self, edges):
        Bh_j = edges.src['Bh']
        e_ij = edges.src['Dh'] - edges.dst['Eh'] # assume l1 distance
        edges.data['e'] = e_ij
        return {'Bh_j':Bh_j, 'e_ij':e_ij}
    ...
    def reduce_func(self, nodes):
        ...
        sigma_ij = torch.sigmoid(e) # sigma_ij = sigmoid(e_ij)
        h = Ah_i + torch.sum( sigma_ij * Bh_j, dim=1 ) / ( torch.sum( sigma_ij, dim=1 ) + 1e-6 )  # hi = Ahi + sum_j eta_ij/sum_j' eta_ij' * Bhj <= dense attention
        # h = Ah_i + torch.sum(e, dim=1) + torch.sum(Bh_j, dim=1)
        return {'h' : h}
