...
class Trainer:
    ...
    def __call__(self):
        ...
        while total_steps < self._max_steps:
            ...
            execute_action_start_time = time.perf_counter()
            if total_steps < self._policy.n_warmup:
                # random_action_start_time = time.perf_counter()
                action = np.stack([self._env.action_space.sample() for _ in range(obs.number_of_nodes())], axis=0)
                # random_action_time = round((time.perf_counter() - random_action_start_time),3)
                # print("random action time:" + str(random_action_time))
            else:
                # selection_action_start_time = time.perf_counter()
                action = self._policy.get_action(obs)
                # selection_action_time = round((time.perf_counter() - selection_action_start_time),3)
                # print("selection action time:" + str(selection_action_time))
            ...
            execute_action_time = round((time.perf_counter() - execute_action_start_time),3)
            # if not total_steps < self._policy.n_warmup:
            #     print("execute action time:" + str(execute_action_time))
            ...
            if total_steps % self._policy.update_interval==0:
                ...
                # train_time = round((time.perf_counter() - train_start_time),3)
                # if not total_steps < self._policy.n_warmup:
                #     print("one step train time:" + str(train_time))
