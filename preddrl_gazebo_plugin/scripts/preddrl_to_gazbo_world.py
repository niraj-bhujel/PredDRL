...
def generate_gzb_world( preddrl_file_name ): 
    rospack1 = RosPack()
    pkg_path = rospack1.get_path('preddrl_simulator')
    xml_scenario =  pkg_path + "/scenarios/" + preddrl_file_name #bo_airport.xml"
    rospack2 = RosPack()
    pkg_path = rospack2.get_path('preddrl_gazebo_plugin')
    gazebo_world =  pkg_path + "/worlds/" + preddrl_file_name.split('.')[0] + ".world" #bo_airport.xml"
    global gzb_world    
...
def generate_launch_file( preddrl_file_name ): 
    rospack2 = RosPack()
    pkg_path = rospack2.get_path('preddrl_gazebo_plugin')
    launch_file =  pkg_path + "/launch/" + preddrl_file_name.split('.')[0] + ".launch" #bo_airport.xml"
    with open(launch_file, 'w') as launch:
        ...
        <include file="$(find gazebo_ros)/launch/empty_world.launch">
             <arg name="world_name" value="$(find preddrl_gazebo_plugin)/worlds/{}.world"/>
         </include>
...
'''.format(preddrl_file_name.split('.')[0])
    print "launch file has been generated: {}".format( launch_file )
 

      
if __name__ == "__main__": 
    preddrl_file_name = raw_input(">> enter preddrl scenaria name: file_name.xml \n")

    # genrate gazebo wolrd 
    generate_gzb_world( preddrl_file_name )     
    generate_launch_file( preddrl_file_name ) 
    print ">> after you launch the scenario using preddrl_simulator, launch the generated world using: "
    print " \" $roslaunch preddrl_gazebo_plugin {}.launch\"  ".format( preddrl_file_name.split('.')[0] )
